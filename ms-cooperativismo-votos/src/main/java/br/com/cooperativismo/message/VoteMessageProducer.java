...
public record VoteMessageProducer(KafkaTemplate<String, VoteEvent> kafkaTemplate){
    ...
    public void send(VoteEvent event) {
        try {
            ...
            log.info("Vote-Message-Producer time={} status=SUCCESS method=#send event={}", DateTimeHelper.getLocalDateTimeFormatted(), event);
            // envia métrica de sucesso
        } catch (RuntimeException e) {
            log.error("Vote-Message-Producer time={} status=ERROR method=#send event={} errorMessage={}", DateTimeHelper.getLocalDateTimeFormatted(), event, e.getMessage());
